...
final class CurrencyNetworkManager {
    ...
    func obtainCurrency() {
        ...
        networkServiсe.get(type: ExchangeRates.self, from: url, parameters: parameters) { answer in
            DispatchQueue.main.async {
                switch answer {
                case .success(let result):
                    ExchangeRatesManager.shared.setExchangeRates(exchangeRates: result)
   ...
   private func getParameters() -> [String: String]? {
        ...
        var parameters = ["currencies": "USD,GBP,EUR,RUB,JPY"]
