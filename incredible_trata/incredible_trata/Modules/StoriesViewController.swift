...
class StoriesViewController: UIViewController {
    ...
    private let colors: [(start: CGColor, end: CGColor)] = [
        (CGColor(UIColor(rgb: 0xF08A4C)), CGColor(UIColor(rgb: 0xE06E61))),
    ...
    private var categoriesWithAmount: [(category: Category, amount: Double)] = [] {
        didSet {
    ...
    private func setData() {
        ...
        var categories: [(category: Category, amount: Double)] = []
    ...
    private func getAmount(for records: [Record]) -> Double {
        var amount: Double = 0
    ...
    private func groupRecords(_ records: [Record]) -> [Category: [Record]] {
        // TODO: force unwrap
        var groupedRecords = Dictionary(grouping: records, by: { $0.category! })
        for (category, records) in groupedRecords {
