...
class SettingsRecordViewController: UIViewController {
    ...
    @objc
    func noteTextFieldChanged() {
        childrenRecord?.note = noteTextField.text!
    }

    @objc
    func amountTextFieldChanged() {
        childrenRecord?.amount = Double((amountTextField.text)!)!
    }
    ...
    @objc
    func actionLongTap (tap: UILongPressGestureRecognizer) {
