...
class AppearanceViewController: UIViewController {
        ...
        AppearanceSection.appearances.forEach { section in
            ...
            section.rows.forEach { row in
                ...
                if case row = Theme.current {
                   model = AppearanceTableViewCellModel(cellType: row.type, text: row.text, switched: false)
                } else {
                    model = AppearanceTableViewCellModel(cellType: row.type, text: row.text, switched: true)
                }
    ...
    override func viewDidLoad() {
        ...
        navigationController?.navigationBar.titleTextAttributes = [
            NSAttributedString.Key.foregroundColor: Color.addButtonBG
        ]
